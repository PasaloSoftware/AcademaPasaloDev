...
type GoogleRequestMethod = 'GET' | 'POST' | 'DELETE';
type GoogleRequest = {
  url: string;
...
async function listDirectChildFolders(
  client: GoogleRequestClient,
  parentFolderId: string,
  ...
  do {
    const urlBase = `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(
      query,
    ...
    for (const file of response.data.files || []) {
      if (!file.id) continue;
      result.push({ id: file.id, name: String(file.name || '(sin nombre)') });
    }
...
async function deleteFolder(
  ...
  await client.request({
    url: `https://www.googleapis.com/drive/v3/files/${encodeURIComponent(folderId)}?supportsAllDrives=true`,
    method: 'DELETE',
  });
}
...
async function main(): Promise<void> {
  ...
  if (!cycles.length) {
    console.log('No se encontraron course_cycle en el ciclo activo.');
    return;
  }
  ...
  const details: Array<{
    ...
    courseCode: string;
    cycleCode: string;
    removedChildFolders: number;
    createdGroups: Array<{ type: string; folderName: string; count: number }>;
  }> = [];
  ...
  for (const row of cycles) {
    ...
    const existingChildren = await listDirectChildFolders(driveClient, bankFolderId);
    for (const child of existingChildren) {
      await deleteFolder(driveClient, child.id);
      deletedFolders += 1;
    }
    ...
    JSON.stringify(
      {
        rootFolderId,
        courseCyclesParentFolderId,
        rebuiltCourseCycles: rebuilt,
        deletedGroupFolders: deletedFolders,
        details,
      },
      null,
      2,
