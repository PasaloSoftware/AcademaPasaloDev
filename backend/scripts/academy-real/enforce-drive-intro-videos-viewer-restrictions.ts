...
async function getDriveClient(): Promise<GoogleRequestClient> {
  ...
  if (typeof client.request !== 'function') throw new Error('Cliente Drive invalido');
  return client;
}
...
async function main(): Promise<void> {
  ...
  try {
    const [rows] = await conn.query(
      `
      SELECT DISTINCT cc.intro_video_file_id AS introVideoFileId
      ...
      WHERE cc.academic_cycle_id = COALESCE(
        ...
        AND cc.intro_video_file_id IS NOT NULL
        AND cc.intro_video_file_id <> ''
      ORDER BY cc.intro_video_file_id ASC
      `,
    ) as [Array<{ introVideoFileId: string }>, unknown];

    console.log(`[INFO] Intro videos a endurecer: ${rows.length}`);
    let ok = 0;
    let fail = 0;

    for (let i = 0; i < rows.length; i += 1) {
      const fileId = String(rows[i].introVideoFileId || '').trim();
      if (!fileId) continue;
      try {
