...
export default function DatePicker({
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    cells.push({
      day: d,
      inMonth: true,
      dateStr: toDateStr(viewYear, viewMonth, d),
    });
  }
  ...
  return (
    <div className="w-full flex-1">
      ...
      >
        <span
          className={`flex-1 text-left text-base font-normal leading-4 line-clamp-1 ${value ? "text-text-primary" : "text-text-tertiary"}`}
        >
        ...
        <Icon
          name="calendar_month"
          size={16}
          className="text-icon-tertiary shrink-0"
        />
      </button>
          ...
          >
            <div className="self-stretch inline-flex justify-start items-center gap-2">
              ...
              <button
                type="button"
                onClick={prevMonth}
                className="p-0.5 rounded-full flex justify-center items-center hover:bg-bg-secondary transition-colors"
              >
                <Icon
                  name="chevron_left"
                  size={16}
            ...
            <div className="inline-flex justify-center items-center gap-3">
              {DAY_HEADERS.map((d) => (
                <div
                  key={d}
                  className="w-8 h-8 text-center flex items-center justify-center text-text-primary text-sm font-medium leading-4"
                >
                  {d}
                </div>
              ))}
