...
function UpdateVideoModal({
  ...
  const pendingDriveTokenResolveRef = useRef<((token: string) => void) | null>(
    null,
  );
  const pendingDriveTokenRejectRef = useRef<
  ...
  const createBrowserResumableSession = async (input: {
    ...
    const resumableSessionUrl = String(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const startResponse = await classEventService.startRecordingUpload(
        event.id,
        {
          fileName: selectedFile.name,
          mimeType: fileMimeType,
          ...
          xhr.onload = () => {
            if (xhr.status >= 200 && xhr.status < 300) {
              try {
                ...
                reject(new Error("Error al parsear respuesta de Drive"));
              }
            } else {
              reject(
                new Error(`Error subiendo archivo a Drive: ${xhr.status}`),
              );
            }
          };

          xhr.onerror = () =>
            reject(new Error("Error de red al subir a Drive"));
      ...
      const finalizeResponse = await classEventService.finalizeRecordingUpload(
        event.id,
        {
          uploadToken: startResponse.uploadToken,
          fileId,
      ...
      if (
        ...
        err.message.includes("Ya existe una carga activa")
      ) {
        await loadUploadStatus();
      }
      setError(
        err instanceof Error ? err.message : "Error al subir la grabacion",
      );
    } finally {
      setSaving(false);
    }
    ...
    if (!isAllowedVideo(file)) {
      setError(
        file.size > MAX_VIDEO_SIZE
          ? "El video excede el tamaño máximo de 10 GB"
          : "Formato no permitido. Usa MP4 o MKV.",
      );
      return;
    }
  ...
  const canSubmit =
    !!selectedFile && !saving && !statusLoading && !activeUpload;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-lg bg-bg-primary rounded-2xl shadow-xl flex flex-col">
        ...
        <div className="p-6 flex flex-col gap-5">
          {isFirstUpload && !activeUpload && (
            <div className="rounded-lg bg-bg-info-primary-light p-3 outline outline-1 outline-muted-indigo-200 flex flex-col gap-1.5">
              ...
              <span className="text-text-secondary text-xs leading-4">
                Sube el video final para publicarlo por primera vez en la
                plataforma.
              </span>
            </div>
          )}
          ...
          {activeUpload && (
            <div className="rounded-lg bg-bg-secondary p-3 outline outline-1 outline-stroke-primary flex flex-col gap-2">
              <p className="text-text-primary text-sm leading-4">
                ...
                {activeUpload.mode === "initial" ? "inicial" : "reemplazo"}).
              </p>
          ...
          {selectedFile && (
            <div className="p-3 bg-bg-secondary rounded-lg flex flex-col gap-3">
              <div className="flex items-center gap-1">
                ...
                <img
                  src={getVideoFileIcon(selectedFile.name)}
                  alt="video"
          ...
          {/* Error */}
          {error && (
            <span className="text-text-error-primary text-sm">{error}</span>
          )}

          {/* Warning */}
          {saving && (
            <span className="text-text-tertiary text-xs leading-3">
              No cierres ni recargues esta pestaña mientras la carga esté en
              progreso.
            </span>
          )}
        </div>
          ...
          <Modal.Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                await onDeleted();
                router.push(
                  `/plataforma/curso/${event.courseCycleId}/evaluacion/${event.evaluationId}`,
                );
              } catch (err) {
                showToast({
                  type: "error",
...
function ContextMenuButton({
  ...
  const handleCopySummary = async () => {
    ...
    const profNames =
      event.professors.map((p) => `${p.firstName} ${p.lastName1}`).join(", ") ||
      "Sin asignar";
    const summary = [
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 w-48 p-1 bg-bg-primary rounded-lg shadow-[2px_4px_4px_0px_rgba(0,0,0,0.05)] outline outline-1 outline-offset-[-1px] outline-stroke-secondary flex flex-col">
          <button
            onClick={() => {
              setOpen(false);
              setModalState({ type: "editInfo", event, reload: reloadEvent });
            }}
...
export default function VideoPageContent({
  ...
  const renderActions = useCallback(
    (event: ClassEvent, reloadEvent: () => Promise<void>) => {
