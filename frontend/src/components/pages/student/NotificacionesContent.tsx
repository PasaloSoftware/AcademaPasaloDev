...
function getCtaLabel(type: string): string | null {
  switch (type) {
    case 'NEW_MATERIAL':
...
function NotificationCard({
  ...
  return (
    ...
    >
      {/* Icon */}
      <div className={`p-3 ${style.bgClass} rounded-full flex justify-center items-center`}>
...
export default function NotificacionesContent() {
  ...
  const handleNotificationAction = async (notification: NotificationItem) => {
    ...
    switch (type) {
      case 'NEW_MATERIAL':
      case 'MATERIAL_UPDATED': {
        if (target?.courseCycleId && target?.evaluationId && target?.classEventId) {
          router.push(
      ...
      case 'CLASS_REMINDER': {
        if (target?.classEventId) {
          try {
            const event = await classEventService.getEventDetail(target.classEventId);
      ...
      case 'AUDIT_EXPORT_READY': {
        if (target?.auditExportJobId) {
          try {
            const blob = await auditService.downloadExportJob(target.auditExportJobId);
