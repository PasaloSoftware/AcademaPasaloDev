...
export default function VideoPageLayout({ cursoId, evalId, eventId, resolveNames, renderActions, canUploadMaterials }: VideoPageLayoutProps) {
  ...
  const [contextMenuMat, setContextMenuMat] = useState<{ mat: ClassEventMaterial; x: number; y: number } | null>(null);
  ...
  const uploadSingleFile = useCallback(
    (entry: StagedFileEntry, folderId: string): Promise<void> =>
      new Promise((resolve, reject) => {
        ...
        setStagedFiles((prev) =>
          prev.map((f) =>
            f.id === entry.id ? { ...f, status: 'uploading' as const, startTime: Date.now(), xhr } : f,
          ),
        );
        ...
        xhr.onload = () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            setStagedFiles((prev) =>
              prev.map((f) =>
                f.id === entry.id ? { ...f, status: 'uploaded' as const, progress: 100, xhr: null } : f,
              ),
            );
            resolve();
          } else {
  ...
  const handleStartUpload = useCallback(async () => {
    ...
    try {
      ...
      const targetFolder =
        rootFolders.find((f) => f.name.toLowerCase().includes('clase')) || rootFolders[0];
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <div className="flex items-start gap-1">
              <Icon name="school" size={20} className="text-icon-accent-primary" />
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-2">
                        {entry.status === 'uploaded' && (
                          <Icon name="check_circle" size={16} className="text-icon-success-primary" variant="rounded" />
                        )}
                        {entry.status === 'error' && (
                          <Icon name="error" size={16} className="text-text-error-primary" variant="rounded" />
                        )}
                        {entry.status !== 'uploading' && (
                          <>
                            <button
  ...
  return (
    <div className="w-full flex flex-col gap-8">
                  ...
                  return (
                    <div key={mat.id} className="relative">
                      <div className="p-3 bg-bg-secondary rounded-lg flex items-center gap-3 hover:bg-bg-tertiary transition-colors w-full">
                        ...
                        >
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img src={iconPath} alt={ext || 'file'} className="w-8 h-8 flex-shrink-0" />
                          <div className="flex-1 min-w-0 flex flex-col gap-1">
                            <div className="flex items-start min-w-0">
                              <span className="text-text-primary text-sm font-normal leading-4 line-clamp-1">{nameOnly}</span>
                              <span className="text-text-primary text-sm font-normal leading-4 flex-shrink-0">{ext}</span>
                              {hiddenMatIds.has(mat.id) && (
                                <Icon name="visibility_off" size={14} className="text-icon-tertiary flex-shrink-0 ml-1" variant="rounded" />
                              )}
                            </div>
                            <span className="text-text-tertiary text-[10px] font-normal leading-3">
                        ...
                        {canUploadMaterials ? (
                          <button
                            onClick={(e) => {
                              ...
                              setContextMenuMat({ mat, x: rect.right, y: rect.bottom });
                            }}
                      ...
                      {contextMenuMat?.mat.id === mat.id && (
                        ...
                        >
                          <button
                            onClick={() => { setContextMenuMat(null); setPreviewIndex(matIdx); setShowPreview(true); }}
                          ...
                          <button
                            onClick={() => { setContextMenuMat(null); materialsService.downloadMaterial(mat.id, fileName); }}
                            className="self-stretch px-2 py-3 rounded inline-flex items-center gap-2 hover:bg-bg-secondary transition-colors"
                          >
                          ...
                          <button
                            onClick={() => { setContextMenuMat(null); setInfoMat(mat); }}
                            className="self-stretch px-2 py-3 rounded inline-flex items-center gap-2 hover:bg-bg-secondary transition-colors"
                          >
                          ...
                          <button
                            onClick={() => { setContextMenuMat(null); setDeleteMat(mat); }}
                            className="self-stretch px-2 py-3 rounded inline-flex items-center gap-2 hover:bg-bg-secondary transition-colors"
                          >
      ...
      {/* Material Info Modal */}
      {infoMat && (() => {
        const infoFileName = infoMat.displayName || infoMat.fileResource.originalName;
        const infoExt = getFileExtension(infoFileName).replace('.', '').toUpperCase();
        const infoNameOnly = getFileNameWithoutExt(infoFileName);
        const infoSize = formatBytes(parseInt(infoMat.fileResource.sizeBytes, 10) || 0);
        const infoModified = new Date(infoMat.fileResource.createdAt).toLocaleDateString('es-PE', {
          day: 'numeric', month: 'long', year: 'numeric', timeZone: 'America/Lima',
        });
        const infoUploaded = new Date(infoMat.createdAt).toLocaleDateString('es-PE', {
          day: 'numeric', month: 'long', year: 'numeric', timeZone: 'America/Lima',
        });

        return (
          <Modal isOpen onClose={() => setInfoMat(null)} title="Información del material" size="md">
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                <span className="text-text-secondary text-xs font-medium leading-4">Nombre</span>
                <span className="text-text-tertiary text-base font-normal leading-4">{infoNameOnly}</span>
              </div>
              <div className="flex flex-col gap-1">
                <span className="text-text-secondary text-xs font-medium leading-4">Tipo</span>
                <span className="text-text-tertiary text-base font-normal leading-4">{infoExt || 'Desconocido'}</span>
              </div>
              <div className="flex flex-col gap-1">
                <span className="text-text-secondary text-xs font-medium leading-4">Tamaño</span>
                <span className="text-text-tertiary text-base font-normal leading-4">{infoSize}</span>
              </div>
              <div className="flex flex-col gap-1">
                <span className="text-text-secondary text-xs font-medium leading-4">Ubicación</span>
              ...
              <div className="flex flex-col gap-1">
                <span className="text-text-secondary text-xs font-medium leading-4">Propietario</span>
                <span className="text-text-tertiary text-base font-normal leading-4">yo</span>
              </div>
              <div className="flex flex-col gap-1">
                <span className="text-text-secondary text-xs font-medium leading-4">Modificado</span>
                <span className="text-text-tertiary text-base font-normal leading-4">{infoModified} por yo</span>
              </div>
              <div className="flex flex-col gap-1">
                <span className="text-text-secondary text-xs font-medium leading-4">Subido</span>
                <span className="text-text-tertiary text-base font-normal leading-4">{infoUploaded} por yo</span>
              </div>
            </div>
          </Modal>
        );
      ...
      <Modal
        ...
        footer={
          <>
            <Modal.Button variant="secondary" onClick={() => setDeleteMat(null)}>
              Cancelar
            </Modal.Button>
            <Modal.Button
              variant="danger"
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  showToast({
                    ...
                    description: 'El material se ocultará hasta que los administradores aprueben la eliminación.',
                  });
                } catch (err) {
